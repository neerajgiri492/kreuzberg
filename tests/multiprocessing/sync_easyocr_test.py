...
from kreuzberg._multiprocessing.sync_easyocr import (
...
def test_process_image_sync_pure_success(sample_image_path: str, mock_easyocr: Mock) -> None:
...
def test_process_image_sync_pure_no_results(sample_image_path: str, mock_easyocr: Mock) -> None:
...
def test_process_image_sync_pure_default_config(sample_image_path: str, mock_easyocr: Mock) -> None:
...
def test_process_image_sync_pure_detail_false(sample_image_path: str, mock_easyocr: Mock) -> None:
    """Test image processing with detail=False config."""
    mock_easyocr.readtext.return_value = ["Hello", "World"]

    # Create a simple mock config with detail=False
    config = Mock()
    ...
    # Mock getattr to return detail=False for lines 127-128 coverage
    def mock_getattr(obj: Any, attr: str, default: Any = None) -> Any:
        if attr == "detail":
            return False  # This triggers lines 127-128
...
def test_process_image_sync_pure_short_result(sample_image_path: str, mock_easyocr: Mock) -> None:
...
def test_process_image_sync_pure_invalid_result(sample_image_path: str, mock_easyocr: Mock) -> None:
...
def test_process_image_sync_pure_exception(sample_image_path: str) -> None:
    ...
    with patch(
        "kreuzberg._multiprocessing.sync_easyocr._get_easyocr_instance", side_effect=Exception("EasyOCR failed")
    ):
...
@pytest.mark.skip("Complex mocking leads to issues")
def test_process_image_sync_pure_advanced_config(sample_image_path: str, mock_easyocr: Mock) -> None:
...
def test_process_image_bytes_sync_pure_success(sample_image_bytes: bytes, mock_easyocr: Mock) -> None:
...
def test_process_image_bytes_sync_pure_cleanup() -> None:
    ...
    with patch("kreuzberg._multiprocessing.sync_easyocr.Image.open") as mock_open:
        mock_image = Mock()
        mock_open.return_value.__enter__.return_value = mock_image

        with patch("kreuzberg._multiprocessing.sync_easyocr.process_image_sync_pure") as mock_process:
...
def test_process_batch_images_sync_pure_success(mock_easyocr: Mock) -> None:
...
def test_process_batch_images_sync_pure_empty_list(mock_easyocr: Mock) -> None:
